...
def redirect_to_wikipedia(request, department_id):
    departments = Departments.objects.get(pk=department_id)
    return redirect(departments.wikipedia_link)
...
from school_store_app.models import Courses
...
def load_courses(request):
    ...
    courses = Courses.objects.filter(department_id=department_id).values()
